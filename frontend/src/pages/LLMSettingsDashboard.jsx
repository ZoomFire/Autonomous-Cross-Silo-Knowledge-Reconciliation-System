    ...
    <section className="agent-page">
        ...
        <div className="agent-grid">
          <div className="panel-card">
            ...
            <div className="hybrid-controls">
              <label>Provider<select value={settingsForm.provider} onChange={(event) => setSettingsForm({ ...settingsForm, provider: event.target.value })}><option>local</option><option>openai</option><option>gemini</option><option>ollama</option><option>custom</option></select></label>
              ...
              <label>Runtime API key<input type="password" value={settingsForm.runtime_api_key || ""} onChange={(event) => setSettingsForm({ ...settingsForm, runtime_api_key: event.target.value })} placeholder="Optional, not stored as plaintext" /></label>
              <label className="checkbox-label"><input type="checkbox" checked={settingsForm.enabled} onChange={(event) => setSettingsForm({ ...settingsForm, enabled: event.target.checked })} /> Enabled</label>
            </div>
          ...
          <div className="panel-card">
            ...
            <div className="agent-log-list">
              {settings.map((item) => <div className="agent-log-row" key={item.settings_id}><strong>{item.provider}</strong><Badge value={item.reasoning_mode} kind="info" /><p>{item.model_name} {item.api_key_masked ? `| key ${item.api_key_masked}` : ""}</p></div>)}
              {!settings.length && <p className="muted">No settings saved yet.</p>}
            </div>
          </div>
        </div>
      )}
      ...
      {canManage && (
        <div className="panel-card">
          ...
          <div className="hybrid-controls">
            ...
            <button className="secondary-button" onClick={saveTemplate} disabled={busy === "template" || !templateForm.name || !templateForm.template_text}><Save size={16} />Save Template</button>
          </div>
          ...
          <div className="agent-history">
            {templates.map((template) => (
              <div className="agent-history-row" key={template.template_id}>
                <div><strong>{template.name}</strong><p>{template.task_type}</p><small>{template.created_at || "Default template"}</small></div>
                <Badge value={template.template_id.startsWith("default-") ? "default" : "custom"} kind="neutral" />
                <div className="history-actions">
                  <button className="icon-button" onClick={() => setTemplateForm(template)} title="View/Edit"><Edit3 size={16} /></button>
                  {!template.template_id.startsWith("default-") && <button className="icon-button danger" onClick={() => removeTemplate(template.template_id)} title="Delete"><Trash2 size={16} /></button>}
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="agent-grid">
        <div className="panel-card">
          ...
          <div className="agent-history">
            {traces.map((trace) => (
              <div className="agent-history-row" key={trace.trace_id}>
                <div><strong>{shortId(trace.trace_id)}</strong><p>{trace.task_type}</p><small>{trace.created_at}</small></div>
                <Badge value={trace.reasoning_mode} kind="info" />
                <Badge value={trace.status} kind={trace.status === "failed" ? "danger" : "success"} />
                <div className="history-actions">
                  <button className="icon-button" onClick={() => viewTrace(trace.trace_id)}>View</button>
                  <button className="icon-button" onClick={() => exportReasoningTraceMarkdown(trace.trace_id)}><Download size={16} /></button>
                </div>
              </div>
            ))}
        ...
        <div className="panel-card">
          ...
          <div className="agent-history">
            {hybridResults.map((item) => (
              <div className="agent-history-row" key={item.result_id}>
                <div><strong>{shortId(item.result_id)}</strong><p>{item.task_type}</p><small>{item.created_at}</small></div>
                ...
                <div className="history-actions">
                  <button className="icon-button" onClick={() => setSelectedTrace({ final_output: item.final_result, validation_result: item.comparison, trace_id: item.trace_id })}>View</button>
                  <button className="icon-button" onClick={() => approveResult(item.result_id, "approved")}><Check size={16} /></button>
                  <button className="icon-button danger" onClick={() => approveResult(item.result_id, "rejected")}><X size={16} /></button>
                </div>
              </div>
            ))}
