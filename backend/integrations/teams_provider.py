...
class TeamsIntegrationProvider(BaseIntegrationProvider):
    def test_connection(self, config: dict) -> dict:
        if not config.get("webhook_url"):
            return provider_response(False, error="Teams live integration is not configured. Use mock mode or provide webhook_url.")
    ...
    def send_notification(self, payload: dict, config: dict) -> dict:
        ...
        if not result["success"]:
            return provider_response(False, response=result, error=result.get("error") or result.get("text") or "Teams webhook request failed.")
        return provider_response(True, "TEAMS-LIVE", "", "sent", result)
